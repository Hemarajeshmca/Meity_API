...
{
    public class FISTransferRegister_DataModel
    {
        public DataConnection MysqlCon;
        MySqlTransaction mysqltrans;
        ErrorMessages ObjErrormsg = new ErrorMessages();
        log4net.ILog logger = log4net.LogManager.GetLogger(typeof(FISTransferRegister_DataModel)); //Declaring Log4Net. 
        string methodName = "";
        public FISTrnregApplication FetchTrnRegister(FISTrnregContext objinvoice, string mysqlcon)
        {
            ...
            objinvoiceRoot.context = new FISTrnregContext();
            objinvoiceRoot.context.Header = new FISTrnregHeader();
            objinvoiceRoot.context.Detail = new List<FISTrnregDetail>();

            MysqlCon = new DataConnection(mysqlcon);
            try
            {

                MySqlCommand cmd = new MySqlCommand("FIS_fetch_transfer_register", MysqlCon.con);
                ...
                {
                    FISTrnregDetail objList = new FISTrnregDetail();
        ...
        public SFISTrnregDocument newTransferRegister(SFISTrnregApplication ObjICDStock, string mysqlcon)
        {
            methodName = MethodBase.GetCurrentMethod().Name;
            DataConnection con = new DataConnection(mysqlcon);
            MysqlCon = new DataConnection(mysqlcon);
            SFISTrnregDocument ObjFarmer = new SFISTrnregDocument();
            ObjFarmer.context = new SFISTrnregContext();
            ObjFarmer.context.Header = new SFISTrnregHeader();
            ObjFarmer.context.Detail = new List<SFISTrnregDetail>();
            try
            {
                int ret = 0; 
                ...
                MySqlCommand cmd = new MySqlCommand("FIS_insupd_transfer_register", MysqlCon.con);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                {
                    MySqlCommand cmdS = new MySqlCommand("FIS_iud_transfer_register", MysqlCon.con);
