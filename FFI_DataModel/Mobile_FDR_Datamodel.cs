...
namespace FFI_DataModel
{
    public class Mobile_FDR_Datamodel
    {
        ...
        public MFMApplication GetallFarmermaster_DB(MFMContext objinvoice, string mysqlcon)
        {
            ...
            DataTable PaytmMasterDt = new DataTable();
            DataTable AEPSMasterDt = new DataTable();
            ...
            objinvoiceRoot.context.PaytmMasterDt = new List<PaytmDetails>();
            objinvoiceRoot.context.AEPSMasterDt = new List<AEPSDetails>();
             MysqlCon = new DataConnection(mysqlcon);
            try
            {
                ...
                if(objinvoice.screen_Id.ToUpper() == "FARMERANDPAWHS")
                {
                    if (ds.Tables.Count > 0)
                    {
                        ...
                        PaytmMasterDt = ds.Tables[5];
                        for (int i = 0; i < PaytmMasterDt.Rows.Count; i++)
                        {
                            PaytmDetails Objlist = new PaytmDetails();
                            Objlist.Out_paytm_masid = Convert.ToInt32(PaytmMasterDt.Rows[i]["Out_paytm_masid"]);
                            Objlist.Out_paytm_fpocode = PaytmMasterDt.Rows[i]["Out_paytm_fpocode"].ToString();
                            Objlist.Out_paytm_type = PaytmMasterDt.Rows[i]["Out_paytm_type"].ToString();
                            Objlist.Out_paytm_merchantId = PaytmMasterDt.Rows[i]["Out_paytm_merchantId"].ToString();
                            Objlist.Out_paytm_tid = PaytmMasterDt.Rows[i]["Out_paytm_tid"].ToString();
                            Objlist.Out_paytm_merchantkey = PaytmMasterDt.Rows[i]["Out_paytm_merchantkey"].ToString();
                            objinvoiceRoot.context.PaytmMasterDt.Add(Objlist);

                        }
                        AEPSMasterDt = ds.Tables[6];
                        for (int i = 0; i < AEPSMasterDt.Rows.Count; i++)
                        {
                            AEPSDetails AEPSObjlist = new AEPSDetails();
                            AEPSObjlist.Out_AEPS_masid = Convert.ToInt32(AEPSMasterDt.Rows[i]["Out_AEPS_masid"]);
                            AEPSObjlist.Out_AEPS_fpocode = AEPSMasterDt.Rows[i]["Out_AEPS_fpocode"].ToString();
                            AEPSObjlist.Out_AEPS_type = AEPSMasterDt.Rows[i]["Out_AEPS_type"].ToString();
                            AEPSObjlist.Out_AEPS_merchantId = AEPSMasterDt.Rows[i]["Out_AEPS_merchantId"].ToString();
                            AEPSObjlist.Out_AEPS_tid = AEPSMasterDt.Rows[i]["Out_AEPS_tid"].ToString();
                            AEPSObjlist.Out_AEPS_merchantkey = AEPSMasterDt.Rows[i]["Out_AEPS_merchantkey"].ToString();
                            objinvoiceRoot.context.AEPSMasterDt.Add(AEPSObjlist);
